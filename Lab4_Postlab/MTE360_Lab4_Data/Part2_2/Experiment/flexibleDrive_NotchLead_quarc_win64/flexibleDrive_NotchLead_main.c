 ...
 * External functions *
 *====================*/
EXTERN RT_MODEL * flexibleDrive_NotchLead(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(flexibleDrive_NotchLead_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(flexibleDrive_NotchLead_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://flexibleDrive_NotchLead:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: flexibleDrive_NotchLead -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (flexibleDrive_NotchLead_DW.HILReadEncoderTimebase_Task, (t_clock)
         flexibleDrive_NotchLead_P.HILReadEncoderTimebase_Clock, 1000.0, -1)
        ;
      if (result == 0) {
